...
@Singleton
public class MqPubSubSender implements MQSendService {
	protected Logger logger = LoggerFactory.getLogger(MqPubSubSender.class);
	...
	public void send(Topic topic, String messageType, Message message) {
		...
		Long result = getRedis().publish(topic.getParentType() + messageType, JsonUtil.toJson(message));
		logger.debug(
