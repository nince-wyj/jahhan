...
abstract class Chunk<T> implements Slice<T>, Serializable {
	...
	/**
	 * Creates a new {@link Chunk} with the given content and the given
	 * governing {@link Pageable}.
	 * 
	 * @param content
	...
	public Chunk(List<T> content, Pageable pageable) {
		...
		Assert.notNull(content, "Content must not be null!", SystemErrorCode.UNKOWN_ERROR);
	...
	protected <S> List<S> getConvertedContent(Converter<? super T, ? extends S> converter) {
		...
		Assert.notNull(converter, "Converter must not be null!", SystemErrorCode.UNKOWN_ERROR);
