...
public abstract class Parameters<S extends Parameters<S, T>, T extends Parameter> implements Iterable<T> {
	...
	public Parameters(Method method) {
		...
		Assert.notNull(method, SystemErrorCode.UNKOWN_ERROR);
	...
	/**
	 * Creates a {@link Parameter} instance for the given
	...
	private final void assertEitherAllParamAnnotatedOrNone() {
		...
		for (T parameter : this.getBindableParameters()) {
			...
			if (parameter.isNamedParameter()) {
				Assert.isTrue(nameFound || index == 0, ALL_OR_NOTHING, SystemErrorCode.UNKOWN_ERROR);
				nameFound = true;
			} else {
				Assert.isTrue(!nameFound, ALL_OR_NOTHING, SystemErrorCode.UNKOWN_ERROR);
			}
