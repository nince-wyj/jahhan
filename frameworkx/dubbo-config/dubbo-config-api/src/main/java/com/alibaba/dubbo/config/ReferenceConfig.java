...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
    ...
    private static final Protocol refprotocol = ExtensionExtendUtil.getAdaptiveExtension(Protocol.class);
    ...
    private final Object finalizerGuardian = new Object() {
        ...
        protected void finalize() throws Throwable {
            ...
            if(! ReferenceConfig.this.destroyed) {
                log.warn("ReferenceConfig(" + url + ") is not DESTROYED when FINALIZE");

                /* 先不做Destroy操作
                try {
                    ReferenceConfig.this.destroy();
                } catch (Throwable t) {
                        log.warn("Unexpected err when destroy invoker of ReferenceConfig(" + url + ") in finalize method!", t);
                }
                */
            }
        }
    ...
    public synchronized void destroy() {
        ...
        try {
            ...
            log.warn("Unexpected err when destroy invoker of ReferenceConfig(" + url + ").", t);
        }
        invoker = null;
        ref = null;
    }
    ...
    private void init() {
        ...
        if (resolve != null && resolve.length() > 0) {
        	...
        	if (log.isWarnEnabled()) {
        		if (resolveFile != null && resolveFile.length() > 0) {
        			log.warn("Using default dubbo resolve file " + resolveFile + " replace " + interfaceName + "" + resolve + " to p2p invoke remote service.");
        		} else {
        			log.warn("Using -D" + interfaceName + "=" + resolve + " to p2p invoke remote service.");
        		}
    		}
        }
        if (consumer != null) {
	...
	private T createProxy(Map<String, String> map) {
		...
		if (isJvmRefer) {
            ...
            if (log.isInfoEnabled()) {
                log.info("Using injvm service " + interfaceClass.getName());
            }
		} else {
        ...
        if (log.isInfoEnabled()) {
            log.info("Refer dubbo service " + interfaceClass.getName() + " from url " + invoker.getUrl());
        }
        // 创建服务代理
