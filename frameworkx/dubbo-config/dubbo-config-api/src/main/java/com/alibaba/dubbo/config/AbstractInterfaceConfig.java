...
public abstract class AbstractInterfaceConfig extends AbstractMethodConfig {
	...
	protected void checkApplication() {
		// 兼容旧版本
		if (application == null) {
			String applicationName = BaseConfiguration.SERVICE;
			...
			for (RegistryConfig config : registries) {
				...
				if (address != null && address.length() > 0 && !RegistryConfig.NO_AVAILABLE.equalsIgnoreCase(address)) {
					...
					if (!map.containsKey("protocol")) {
						if (ExtensionExtendUtil.hasExtension(RegistryFactory.class, "remote")) {
		...
		if (ConfigUtils.isNotEmpty(mock)) {
			if (mock.startsWith(Constants.RETURN_PREFIX)) {
				...
				Class<?> mockClass = ConfigUtils.isDefault(mock)
						? ReflectUtils.forName(interfaceClass.getName() + "Mock") : ReflectUtils.forName(mock);
