...
public class NetUtils {
	...
	private static Map<Integer, Integer> portMapping;

	public static int getMappingPort(int containerPort) {
		if (null == portMapping) {
			portMapping = new HashMap<>();
			String portMappingsString = System.getenv("PORT_MAPPING");
			String[] portMappingS = portMappingsString.split(",");
			for (String portMappingString : portMappingS) {
				String[] split = portMappingString.split(":");
				portMapping.put(Integer.valueOf(split[1]), Integer.valueOf(split[0]));
			}
		}
		return null == portMapping.get(containerPort) ? containerPort : portMapping.get(containerPort);
	}
}