 ...
 */
package net.jahhan.extension.codec2;
...
public class ExchangeCodec extends TelnetCodec {
	...
	public void encode(Channel channel, ChannelBuffer buffer, Object msg) throws IOException {
		...
		// System.out.println(">>>>>>>>>>>>>>>>>>>>>> the resulting byte size of
		// encoding is " + buffer.readableBytes());
		if (log.isTraceEnabled()) {
			log.trace("the resulting byte size of encoding is " + buffer.readableBytes());
		}
			...
			if (!res.isEvent() && res.getStatus() != Response.BAD_RESPONSE) {
				...
				if (t instanceof ExceedPayloadLimitException) {
					...
					try {
						...
						log.warn("Failed to send bad_response info back: " + t.getMessage() + ", cause: "
								+ e.getMessage(), e);
					}
				} else {
					// FIXME 在Codec中打印出错日志？在IoHanndler的caught中统一处理？
					log.warn("Fail to encode response: " + res + ", send bad_response info instead, cause: "
							+ t.getMessage(), t);
					try {
