...
import net.jahhan.extension.threadPool.AbortPolicyWithReport;
...
public class ConnectionOrderedChannelHandler extends WrappedChannelHandler {
	...
	public ConnectionOrderedChannelHandler(ChannelHandler handler, URL url) {
		...
		connectionExecutor = new ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS,
				new LinkedBlockingQueue<Runnable>(
						url.getPositiveParameter(Constants.CONNECT_QUEUE_CAPACITY, Integer.MAX_VALUE)),
				new NamedThreadFactory(threadName, true), new AbortPolicyWithReport(threadName, url)); // FIXME
	...
	public void disconnected(Channel channel) throws RemotingException {
		try {
			...
			throw new ExecutionException("disconnected event", channel,
	...
	private void checkQueueLength() {
		if (connectionExecutor.getQueue().size() > queuewarninglimit) {
			log.warn("", new IllegalThreadStateException("connectionordered channel handler `queue size: "
