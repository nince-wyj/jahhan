...
public class TelnetHandlerAdapter extends ChannelHandlerAdapter implements TelnetHandler {
	...
	public String telnet(Channel channel, String message) throws RemotingException {
		...
		if (command.length() > 0) {
			if (ExtensionExtendUtil.hasExtension(TelnetHandler.class, command)) {
				try {
					String result = ExtensionExtendUtil.getExtension(TelnetHandler.class, command).telnet(channel, message);
					if (result == null) {
						return null;
					}
					buf.append(result);
