	...
	public AbstractClient(URL url, ChannelHandler handler) throws RemotingException {
		...
		try {
			...
			if (log.isInfoEnabled()) {
				log.info("Start " + getClass().getSimpleName() + " " + NetUtils.getLocalAddress()
		...
		executor = (ExecutorService) ExtensionExtendUtil.getExtension(DataStore.class).get(Constants.CONSUMER_SIDE,
				Integer.toString(url.getPort()));
		ExtensionExtendUtil.getExtension(DataStore.class).remove(Constants.CONSUMER_SIDE, Integer.toString(url.getPort()));
	}
	...
	protected void connect() throws RemotingException {
		...
		try {
			...
			if (!isConnected()) {
				...
				if (log.isInfoEnabled()) {
					log.info("Successed connect to server " + getRemoteAddress() + " from "
			...
			throw new RemotingException(this,
					"Failed connect to server " + getRemoteAddress() + " from " + getClass().getSimpleName() + " "
							+ NetUtils.getLocalHost() + " using dubbo version " + Version.getVersion() + ", cause: "
							+ e.getMessage(),
					e);
		} finally {
			connectLock.unlock();
