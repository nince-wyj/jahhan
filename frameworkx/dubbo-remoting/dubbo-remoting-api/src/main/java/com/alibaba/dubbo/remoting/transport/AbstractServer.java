...
public abstract class AbstractServer extends AbstractEndpoint implements Server {
	...
	public AbstractServer(URL url, ChannelHandler handler) throws RemotingException {
		...
		try {
			...
			if (log.isInfoEnabled()) {
				log.info("Start " + getClass().getSimpleName() + " bind " + getBindAddress() + ", export "
						+ getLocalAddress());
			}
		} catch (Throwable t) {
		...
		executor = (ExecutorService) ExtensionExtendUtil.getExtension(DataStore.class)
				.get(Constants.EXECUTOR_SERVICE_COMPONENT_KEY, Integer.toString(url.getPort()));
	}
	...
	public void close() {
		if (log.isInfoEnabled()) {
			log.info("Close " + getClass().getSimpleName() + " bind " + getBindAddress() + ", export "
					+ getLocalAddress());
		}
	...
	public void connected(Channel ch) throws RemotingException {
		...
		if (accepts > 0 && channels.size() > accepts) {
			log.error("Close channel " + ch + ", cause: The server " + ch.getLocalAddress()
	...
	public void disconnected(Channel ch) throws RemotingException {
		...
		if (channels.size() == 0) {
			log.warn(
					"All clients has discontected from " + ch.getLocalAddress() + ". You can graceful shutdown now.");
		}
