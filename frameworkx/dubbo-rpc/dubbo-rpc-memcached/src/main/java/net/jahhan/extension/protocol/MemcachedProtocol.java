...
public class MemcachedProtocol extends AbstractProtocol {
	...
	public <T> Invoker<T> refer(final Class<T> type, final URL url) throws JahhanException {
		try {
					...
					try {
						if (get.equals(invocation.getMethodName())) {
							...
							throw new UnsupportedOperationException(
									"Unsupported method " + invocation.getMethodName() + " in memcached service.");
						}
					} catch (Throwable t) {
						JahhanException re = new JahhanException(
								"Failed to invoke memcached service method. interface: " + type.getName() + ", method: "
			...
			throw new JahhanException("Failed to refer memcached service. interface: " + type.getName() + ", url: "
