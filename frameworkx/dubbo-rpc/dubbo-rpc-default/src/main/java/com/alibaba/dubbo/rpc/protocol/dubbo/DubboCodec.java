...
import net.jahhan.extension.codec2.ExchangeCodec;
...
public class DubboCodec extends ExchangeCodec implements Codec2 {
	...
	protected void encodeRequestData(Channel channel, ObjectOutput out, Object data) throws IOException {
		...
		// TODO
		String dubboVersion = (String) channel.getAttribute("dubbo_version");
		if (dubboVersion.startsWith("x")) {
	...
	private boolean containComplexArguments(RpcInvocation invocation) {
		for (int i = 0; i < invocation.getParameterTypes().length; i++) {
			if (invocation.getArguments()[i] == null
					|| invocation.getParameterTypes()[i] != invocation.getArguments()[i].getClass()) {
				return true;
			}
		}
		return false;
