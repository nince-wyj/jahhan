package net.jahhan.demo.dao.abstr;
...
public abstract class AbstractTUserDemoInfoDao implements TUserDemoInfoDao {
	...
	public int addTUserDemoInfo(TUserDemoInfo tUserDemoInfo){
		...
		if(ret > 0){
			dbContext.publishDataModifyEvent(dataSource, session.getConnection(), tUserDemoInfo, EventOperate.INSERT, pkToString(tUserDemoInfo));
		}
		return ret;
	}
	...
	*/
	public int resetAllTUserDemoInfo(long userId,TUserDemoInfo tUserDemoInfo){
		...
		if(ret > 0){
			dbContext.publishDataModifyEvent(dataSource, session.getConnection(), tUserDemoInfo, EventOperate.UPDATE, pkToString(tUserDemoInfo));
		}
		return ret;
	}
	...
	*/		
	public int updatePartTUserDemoInfo(long userId,TUserDemoInfo tUserDemoInfo){
		...
		if(ret > 0){
			dbContext.publishDataModifyEvent(dataSource, session.getConnection(), tUserDemoInfo, EventOperate.PART_MODIFY, pkToString(tUserDemoInfo));
		}
		return ret;
	}
	...
	*/	
	 ...
	 */
	public int updatePartByIds(List<Long> userIds,TUserDemoInfoPage conditionTUserDemoInfo,TUserDemoInfo newTUserDemoInfo){
		...
		if(ret > 0){
			...
			for(Long userId:userIds){
				tUserDemoInfo.setUserId(userId);
				dbContext.publishDataModifyEvent(dataSource, session.getConnection(), tUserDemoInfo, EventOperate.OTHER_MODIFY, pkToString(tUserDemoInfo));
			}
		}
		return ret;
	}
	...
	/** 分页查询 **/
	public PagedResult<TUserDemoInfo> pagedResultOfListTUserDemoInfo(
			TUserDemoInfoPage tUserDemoInfo) {
	...
	public List<TUserDemoInfo> listTUserDemoInfoByIds(List<Long> userIds){
		//如果不走缓存，就直接从数据库中取
		if(!this.isCachable()){
			return this.listTUserDemoInfoByIdsFromDB(userIds);
		}
		if(CollectionUtils.isEmpty(userIds)){
			return new ArrayList<TUserDemoInfo>();
		}
