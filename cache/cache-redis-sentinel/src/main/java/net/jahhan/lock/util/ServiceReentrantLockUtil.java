...
public class ServiceReentrantLockUtil {
	...
	public static DistributedLock lock(String lockName) {
		DistributedLock distributedLock = ((RedisVariable) RedisVariable.getThreadVariable("redis")).getServiceLock(getKey(lockName));
		if (null == distributedLock) {
			distributedLock = new ServiceReentrantLock(RedisConstants.COMMON, getKey(lockName), ttlSec);
			((RedisVariable) RedisVariable.getThreadVariable("redis")).setServiceLock(getKey(lockName), distributedLock);
		}
