...
public class AuthenticationProviderFilter implements Filter {
	...
	public Result invoke(Invoker<?> invoker, Invocation invocation) throws JahhanException {
		...
		if (null != request && null != response) {
			...
			HttpAssert.isTrue(defineType.equals(requestType), "接口定义与请求不符合！", HttpStatus.SC_BAD_REQUEST,
			...
			if (null != userAuthentication) {
				HttpAssert.notNull(user, "用户未登陆！", HttpStatus.SC_BAD_REQUEST, JahhanErrorCode.NO_AUTHORITY);
			}
			...
			if (null != serviceAuthentication) {
				HttpAssert.notNull(service, "服务未登陆！", HttpStatus.SC_BAD_REQUEST, JahhanErrorCode.NO_AUTHORITY);
			}
			if (!authenticationVariable.isCrypt()) {
				HttpAssert.isTrue(
						BaseConfiguration.IS_DEBUG || (null == userAuthentication && null == serviceAuthentication)
			...
			if (authenticationVariable.isNoneToken() && !authenticationVariable.isDocRequest() && null == noneToken) {
				HttpAssert.isTrue(null == noneToken, "无token错误！！", HttpStatus.SC_BAD_REQUEST, JahhanErrorCode.NO_AUTHORITY);
			}
			if (authenticationVariable.isFirstSingleToken() && !BaseConfiguration.IS_DEBUG && null == noneToken) {
				...
				HttpAssert.notNull(firstToken, "鉴权失败！", HttpStatus.SC_UNAUTHORIZED, JahhanErrorCode.NO_AUTHORITY);
			} else {
				if (!authenticationVariable.isDocRequest() && !BaseConfiguration.IS_DEBUG && null == noneToken) {

					HttpAssert.isTrue(null != user || null != service, "无token错误！", HttpStatus.SC_BAD_REQUEST,
